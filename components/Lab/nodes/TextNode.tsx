...
export const TextNode = memo(function TextNode({ data, id, selected }: NodeProps) {
  ...
  return (
    <div
      className={`xt-canvas-node xt-canvas-node--text ${selected ? 'is-selected' : ''}`}
      style={{
    ...
    >
      ...
      <div className="xt-canvas-node-header">
        <span className="xt-canvas-node-type">TEXT</span>
      </div>
      ...
      {editingField === 'label' ? (
        ...
        <div className="xt-canvas-node-title" onDoubleClick={() => startEdit('label', d.label ?? '')}>{d.label || 'Untitled'}</div>
      )}
      ...
      {editingField === 'content' ? (
        <textarea
          ...
          value={draft}
          onChange={e => setDraft(e.target.value)}
          onBlur={() => commitEdit('content')}
          onKeyDown={e => { if (e.key === 'Escape') setEditingField(null); }}
          onPointerDown={e => e.stopPropagation()}
          style={{ minHeight: 40 }}
        />
      ) : (
        <div className="xt-canvas-node-body" onClick={() => startEdit('content', d.content ?? '')}>
          {d.content ? (
            <>{d.content.slice(0, 120)}{d.content.length > 120 ? '...' : ''}</>
          ) : (
            <span className="xt-canvas-node-body--empty">Click to write...</span>
          )}
        </div>
      )}
    </div>
