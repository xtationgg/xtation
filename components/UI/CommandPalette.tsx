...
export const CommandPalette: React.FC<CommandPaletteProps> = ({
  ...
  const allCommands = useMemo<CommandEntry[]>(
    () => [
      { id: 'nav-home', label: 'Home', group: 'Navigate', action: () => nav(ClientView.HOME) },
      ...
      { id: 'nav-uikit', label: 'UI Kit', group: 'Navigate', action: () => nav(ClientView.UI_KIT) },
      {
        id: 'act-assistant',
        label: 'Toggle Assistant',
        group: 'Actions',
        keywords: 'quests bot ai open close',
        action: () => {
          onToggleAssistant();
          onClose();
        },
    ...
    [isRunning, nav, onClose, onToggleAssistant, startSession, stopSession]
  );
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-start justify-center pt-[14vh] px-4">
      ...
      {/* Panel */}
      <div className="relative w-full max-w-xl rounded-xl border border-[color-mix(in_srgb,var(--app-text)_14%,transparent)] bg-[var(--app-panel-2)] shadow-2xl overflow-hidden">
        {/* Search row */}
        <div className="flex items-center gap-3 px-4 py-3 border-b border-[color-mix(in_srgb,var(--app-text)_8%,transparent)]">
          <span className="text-[var(--app-muted)] text-sm shrink-0 select-none">⌘</span>
          <input
            ref={inputRef}
            value={query}
            ...
            className="flex-1 bg-transparent text-[var(--app-text)] text-[11px] font-mono uppercase tracking-[0.12em] placeholder:text-[var(--app-muted)] placeholder:normal-case placeholder:tracking-normal outline-none"
          />
          <kbd className="hidden sm:inline-flex items-center rounded border border-[color-mix(in_srgb,var(--app-text)_18%,transparent)] px-1.5 py-0.5 text-[9px] uppercase tracking-[0.14em] text-[var(--app-muted)] font-mono shrink-0">
            ESC
          </kbd>
        </div>
        ...
        <div ref={listRef} className="xt-scroll max-h-[52vh] overflow-y-auto py-1.5">
          {filtered.length === 0 ? (
            ...
            Object.entries(groups).map(([group, cmds]) => (
              <div key={group}>
                <div className="px-4 pt-2 pb-1 text-[9px] uppercase tracking-[0.24em] text-[var(--app-muted)]">
                  {group}
                </div>
                {cmds.map((cmd) => {
                  ...
                  return (
                    ...
                    >
                      {isActive ? (
                        <span className="h-1 w-1 rounded-full bg-[var(--app-accent)] shrink-0" />
                      ) : (
        ...
        {/* Footer hints */}
        <div className="flex items-center gap-4 border-t border-[color-mix(in_srgb,var(--app-text)_8%,transparent)] px-4 py-2 text-[9px] uppercase tracking-[0.16em] text-[var(--app-muted)]">
